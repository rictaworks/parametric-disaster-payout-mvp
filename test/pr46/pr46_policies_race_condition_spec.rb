...
RSpec.describe "PR46（補足）: 同時申込に対する二重契約防止の実スレッド検証", type: :model do
    ...
    threads = Array.new(2) do |i|
      Thread.new do
        ActiveRecord::Base.connection_pool.with_connection do
          ActiveRecord::Base.connection.enable_query_cache! # HTTPリクエストと同様にクエリキャッシュを有効化
  ...
  it "ほぼ同時の2リクエストのうち、重複と判定される側は、SQLite環境でも制御された409 duplicate_policy相当の結果（Result）になること" do
  ...
  it "参考（修正後の実挙動の記録）: 実際には片方が201相当で成功し、もう片方は未処理のActiveRecord::StatementInvalidを送出せず制御された409を返すこと" do
    outcomes = run_two_concurrent_requests
    ...
    expect(raised_errors.length).to eq(0)

    conflict_errors = outcomes.select { |o| o.is_a?(ValidateAndCreatePolicy::Result) && !o.success? }
    expect(conflict_errors.length).to eq(1)
    expect(conflict_errors.first.status).to eq(:conflict)
  end
  ...
  it "重複契約が存在しない状態でデータベース例外（StatementInvalid）が発生した場合は、409に変換せず例外を再送出すること" do
    recaptcha_double = instance_double(RecaptchaVerifier, valid?: true)
    allow(RecaptchaVerifier).to receive(:new).and_return(recaptcha_double)

    service = build_service
    allow(service).to receive(:create_policy_within_lock).and_raise(ActiveRecord::StatementInvalid.new("Simulated database timeout or foreign key error"))

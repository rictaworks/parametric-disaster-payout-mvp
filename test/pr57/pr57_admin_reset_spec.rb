...
#
# 実行結果は事前に確認済み（作成時点で 21 examples, 0 failures, 1 pending。
# pendingの1件はOWASP A08(CSRF)に関する既知の指摘であり、本ファイル末尾の
# コメントを参照）。
#
...
RSpec.describe "PR#57 管理画面リセットタブ（デモデータ初期化）", type: :request do
  ...
  describe "OWASP A08: Software and Data Integrity Failures（CSRF対策）" do
    # 既知の指摘事項（このテスト作成時点で確認・red）:
    # Admin::HtmlController（Admin::ResetController の親クラス）は
    # ActionController::Base を直接継承しているが `protect_from_forgery` を
    # 呼び出しておらず、CSRFトークン検証（verify_authenticity_token）が
    # 有効化されていない。BASIC認証はブラウザが資格情報を自動的に
    # 再送するため、Cookieに対するSameSite保護の対象外であり、
    # 別オリジンの罠ページから /admin/reset へ自動送信されるCSRF攻撃を
    # 受けるリスクがある（対して Admin::Api::PayoutsController は
    # 明示的に protect_from_forgery を呼んでおり対策済み）。
    # 本テストは実装側の対策が入るまで pending（red）として明示し、
    # 対策が入れば自動的に失敗検出（"fixed" 扱い）されるようにしておく。
    it "本番相当の設定（forgery protection有効時）ではCSRFトークンなしのPOSTが拒否される", pending: "既知の指摘: Admin::HtmlController系にprotect_from_forgeryが未設定のためCSRF保護が効いていない（要修正）" do
